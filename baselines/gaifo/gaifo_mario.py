...
from mario_src.model import PPO
...
env = create_train_env(1,1, SIMPLE_MOVEMENT,output_path=None)
...
if torch.cuda.is_available():
    expert.load_state_dict(torch.load("{}/ppo_super_mario_bros_{}_{}".format('trained_models', 1,1)))
    expert.cuda()
else:
    expert.load_state_dict(torch.load("{}/ppo_super_mario_bros_{}_{}".format('trained_models', 1,1),
                                         map_location=lambda storage, loc: storage))
...
for i in tqdm(range(n_epochs)):
    ...
    if es_ct == 0:
        ...
        break
    #print('scores: ', v_fid, max(v_fids), v_fids)
    if v_fid > max(v_fids):
